...
namespace uncy.model.boardAlt
{
    public struct Move
    {
        public byte fromFile, fromRank, toFile, toRank;
        public char movedPiece, capturedPiece, promotionPiece;
        public bool castlingMoveFlag, enPassantCaptureFlag, doubleSquarePushFlag;

        public Move(byte fromFile, byte fromRank, byte toFile, byte toRank, char movedPiece, char capturedPiece = 'e', char promotionPiece = 'e', bool castlingMoveFlag = false, bool enPassantCaptureFlag = false, bool doubleSquarePushFlag = false)
        {
        ...
        public override string ToString() => $"({movedPiece}: {fromFile}, {fromRank}->{toFile},{toRank})";
    ...
    internal class MoveGenerator
    {
        ...
        private static List<Move> GeneratePseudoMovesForPawn(int file, int rank, Board board)
        {
            ...
            if (isWhitePiece)
            {
                ...
                // Two Squares Push
                if (moves.Count > 0)  // Means that One Square push was possible. If it wasn't, two square is also not possible, don't have to check further
                {
                ...
                if (!(file-1 < 0 || file-1 >= board.dimensionsOfBoard.Item1 || rank + 1 < 0 || rank + 1 >= board.dimensionsOfBoard.Item2))
                {
                    ...
                    if (!IsPieceWhite(targetPiece)) // Checks if piece is black since this is code is executed for white Pawns
                    {
            ...
            else // Handling for black pieces
            {
                ...
                {
                    char targetPiece = board.board[file - 1, rank + 1];
                    if (IsPieceWhite(targetPiece)) // Checks if piece is white since this is code is executed for black Pawns
                    {
